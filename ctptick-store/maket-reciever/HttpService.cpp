        ...
        switch (ev) {
            case MG_EV_HTTP_REQUEST:
                ...
                if( hm->query_string.len>0) {
                    text = std::string(hm->query_string.p, hm->query_string.len);
                    Application::instance()->getLogger().debug("query_string: " + text);
                }
                if( hm->body.len>0) {
                    text = std::string(hm->body.p, hm->body.len);
                    Application::instance()->getLogger().debug("body: " + text);
                }
...
//                    }
                ...
                } else if (mg_vcmp(&hm->uri, "/ctp/market/subscribe") == 0) { //订阅行情
                    http->handle_market_subcribe(nc, hm);
                } else if (mg_vcmp(&hm->uri, "/ctp/market/unsubscribe") == 0) { //取消订阅行情
                    http->handle_market_unsubcribe(nc, hm);
...
//订阅行情
void HttpService::handle_market_subcribe(struct mg_connection *nc, struct http_message *hm ){
    MdApi* api = Controller::instance()->getMarketApi();
    if( !check_access_allow(nc,hm) ) {
        send_http_response_error(nc, Error_AccessDenied, "Error_AccessDenied");
        return ;
    }

    std::string instruments;
    instruments = http_get_var(hm,"instruments"); // ExchangeID + OrderSysID  (szf.12212)

    // split instrument and trim it
    std::vector< std::string > ss;
    boost::split(ss,instruments,boost::is_any_of(","));
    ...
    if( ss.size() == 0){
        send_http_response_error(nc,Error_ParameterInvalid,"param invalid");
        return ;
    }

    api->subscribeMarketData(ss);
...
//取消订阅行情
void HttpService::handle_market_unsubcribe(struct mg_connection *nc, struct http_message *hm ){
    MdApi* api = Controller::instance()->getMarketApi();
    if( !check_access_allow(nc,hm) ) {
        send_http_response_error(nc, Error_AccessDenied, "Error_AccessDenied");
        return ;
...
bool HttpService::open(){
	...
	std::string http_port  = cfgs_.get_string("http_service.port","8000");
...
void HttpService::handle_order_send(struct mg_connection *nc, struct http_message *hm ){
    ...
    Json::Value data = defaultResponseJsonData();
    data["result"] = TdApi::instance().sendOrder(req).order_id;
    std::string text = data.toStyledString();
    send_http_response(nc,text,defaultResponseHeaders());
}
...
bool HttpService::check_access_allow(struct mg_connection *nc, struct http_message *hm){
    std::string code = Application::instance()->getConfig().get_string("access_code");
    if ( code.size()){
        std::string text = http_get_var(hm,"access_code");
        if ( text != code){
            return false;
        }
    }
