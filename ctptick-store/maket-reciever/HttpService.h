...
//	void handle_innerdevice_register(struct mg_connection *nc, struct http_message *hm );
	...
	void handle_market_subcribe(struct mg_connection *nc, struct http_message *hm ); //订阅行情
	void handle_market_unsubcribe(struct mg_connection *nc, struct http_message *hm ); //取消订阅行情
...
//    void ev_connect_handler(struct mg_connection *nc, int ev, void *ev_data);
    ...
    bool check_access_allow(struct mg_connection *nc, struct http_message *hm);
private:
	Config 	cfgs_;
