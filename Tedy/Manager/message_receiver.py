...
class MessageReceiver(object):
    ...
    def parse(self,text):
        ...
        for user in self.users:
            ...
            elif isinstance(message,TradePosReport):
                user.onTradePosReport(message)
            elif isinstance(message,TradeEquityReport):
                user.onTradeEquityReport(message)
            elif isinstance(message,KlineUpdateReport):
                user.onKlineUpdateReport(message)
            elif isinstance(message,HostRunningStatus):
                user.onHostRunningStatus(message)
